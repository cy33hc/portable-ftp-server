 ...
 */
public class UNZP extends AbstractCommand {

	private final Logger LOG = LoggerFactory.getLogger(UNZP.class);
	...
	public void execute(final FtpIoSession session, final FtpServerContext context, final FtpRequest request)
		...
		try {
			...
			if (file == null) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_550_REQUESTED_ACTION_NOT_TAKEN, "UNZP.missing", fileName, file));
				return;
			}
			...
			if (!file.isFile()) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_550_REQUESTED_ACTION_NOT_TAKEN, "UNZP.invalid", fileName, file));
				return;
			}
			...
			if (!file.isReadable()) {
				session.write(LocalizedDataTransferFtpReply.translate(session, request, context,
						FtpReply.REPLY_550_REQUESTED_ACTION_NOT_TAKEN, "UNZP.permission", fileName, file));
				return;
			}
			...
			ZipFile zipFile = new ZipFile(file.getPhysicalFile().toString());
			...
			session.write(new DefaultFtpReply(FtpReply.REPLY_200_COMMAND_OKAY, "File extracted"));
